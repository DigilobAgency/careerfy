    ...
    class jobsearch_packages_functions {
        ...
        public function add_candidate_resume_id_to_order($candidate_id, $order_id) {
            if ($candidate_id > 0 && $order_id > 0) {
                ...
                $order_cvs = $jobsearch_order->get_meta('jobsearch_order_cvs_list');
                if ($order_cvs != '') {
                ...
                $jobsearch_order->update_meta_data('jobsearch_order_cvs_list', $order_cvs);
        ...
        public function add_candidate_invites_id_to_order($job_ids, $order_id) {
            if ($job_ids != '' && $order_id > 0) {
                $jobsearch_order = wc_get_order($order_id);
                $order_cvs = $jobsearch_order->get_meta('jobsearch_order_invites_list');
                $order_cvs = $order_cvs != '' ? $order_cvs . ',' . $job_ids : $job_ids;
                update_post_meta($order_id, 'jobsearch_order_invites_list', $order_cvs);
                $jobsearch_order->update_meta_data('jobsearch_order_invites_list', $order_cvs);
                $jobsearch_order->save();
            }
        }
        ...
        public function add_candidate_apply_job_id_to_order($candidate_id, $order_id) {
            if ($candidate_id > 0 && $order_id > 0) {
                $jobsearch_order = wc_get_order($order_id);
                $order_apps = $jobsearch_order->get_meta('jobsearch_order_apps_list');
                if ($order_apps != '') {
                ...
                $jobsearch_order->update_meta_data('jobsearch_order_apps_list', $order_apps);
                $jobsearch_order->save();
            }
        }
