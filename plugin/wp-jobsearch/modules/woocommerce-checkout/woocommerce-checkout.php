...
class Jobsearch_Woocommerce_Checkout {
    ...
    public function __construct() {
        ...
        add_action('woocommerce_new_order', array($this, 'trigger_after_checkout_process'), 10, 2); // replacement of "woocommerce_checkout_order_processed" hook
    ...
    public function thankyou_redirectcustom($order_id) {
        ...
        $order_attach_with = $order_obj->get_meta('jobsearch_order_attach_with');
        if ($order_attach_with == 'package' && apply_filters('jobsearch_wc_order_complete_redirect_dashboard', true, $order_id)) {
            $payment_method = $order_obj->get_payment_method();
            if ($payment_method != 'bacs' && $payment_method != 'cod') {
                $order_job_id = $order_obj->get_meta('jobsearch_order_attach_job_id');
                ...
                if ($order_job_id > 0 && get_post_type($order_job_id) == 'job') {
                    ...
                    if ($order_obj->has_status('processing') || $order_obj->has_status('completed')) {
                    ...
                    if (jobsearch_user_is_candidate($user_id)) {
                        $dashmenu_links = isset($jobsearch_plugin_options['cand_dashbord_menu']) ? $jobsearch_plugin_options['cand_dashbord_menu'] : '';
                        $dashmenu_links = apply_filters('jobsearch_cand_dashbord_menu_items_arr', $dashmenu_links);
                        if (isset($dashmenu_links['transactions']) && $dashmenu_links['transactions'] == '1') {
                    ...
                    if (jobsearch_user_is_employer($user_id)) {
                        $dashmenu_links = isset($jobsearch_plugin_options['emp_dashbord_menu']) ? $jobsearch_plugin_options['emp_dashbord_menu'] : '';
                        $dashmenu_links = apply_filters('jobsearch_emp_dashbord_menu_items_arr', $dashmenu_links);
                        if (isset($dashmenu_links['transactions']) && $dashmenu_links['transactions'] == '1') {
    ...
    public function trigger_after_checkout_process($order_id, $order) {
        global $woocommerce;

        $jobsearch_order = $order;
            ...
            if ($prod_attach_with == 'package') {
                ...
                if ($package_name != '' && is_object($product_package_obj)) {
                    ...
                    if ($order_pkg_type == 'feature_job') {
                        ...
                        $jobsearch_order->update_meta_data('jobsearch_order_too_feature', 'yes');
                        $jobsearch_order->update_meta_data('jobsearch_order_too_feature_pid', $package_id);
                    }
                    ...
                    if (isset($_COOKIE['wc_checkout_package_' . $product_id . '_job'])) {
                        ...
                        $jobsearch_order->update_meta_data('jobsearch_order_attach_job_id', $job_id);
                    ...
                    $jobsearch_order->update_meta_data('jobsearch_order_attach_with', 'package');
                    $jobsearch_order->update_meta_data('jobsearch_order_package', $package_id);
                    $jobsearch_order->update_meta_data('jobsearch_order_user', $user_id);
                    $jobsearch_order->update_meta_data('_customer_user', $user_id);
                    $jobsearch_order->set_customer_id($user_id);
                    $jobsearch_order->update_meta_data('jobsearch_order_transaction_type', 'paid');
                    $jobsearch_order->save();
        ...
        if ($pckgs_count == 1) {
            $fetured_pkg_too = $jobsearch_order->get_meta('jobsearch_order_too_feature');
            if ($fetured_pkg_too == 'yes') {
                ...
                $jobsearch_order->update_meta_data('jobsearch_order_too_feature', '');
                $jobsearch_order->update_meta_data('jobsearch_order_too_feature_pid', '');
                $jobsearch_order->save();
    ...
    public function woocommerce_order_status_complete($order_id) {
        ...
        $order_type = $jobsearch_order->get_meta('jobsearch_order_attach_with');
        $order_user = $jobsearch_order->get_meta('jobsearch_order_user');
        ...
        if ($order_type == 'package') {
            ...
            $order_package = $jobsearch_order->get_meta('jobsearch_order_package');
            $order_package_obj = get_post($order_package);

            if (is_object($order_package_obj)) {
                $package_id = $order_package_obj->ID;

                $order_job_id = $jobsearch_order->get_meta('jobsearch_order_attach_job_id');
                ...
                if ($order_job_id > 0) {
                    $order_too_feature = $jobsearch_order->get_meta('jobsearch_order_too_feature');
                    if ($order_too_feature == 'yes') {
                        $order_too_feature_pid = $jobsearch_order->get_meta('jobsearch_order_too_feature_pid');
                    ...
                    if ($order_pkg_type == 'job' || $order_pkg_type == 'featured_jobs' || $order_pkg_type == 'featured_tags' || $order_pkg_type == 'emp_allin_one' || $order_pkg_type == 'employer_profile') {
                        ...
                        $order_expiry_time = $jobsearch_order->get_meta('package_expiry_timestamp');
