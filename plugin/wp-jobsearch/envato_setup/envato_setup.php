<?php
/**
 * Envato Theme Import Demo Data Class
 *
...
if (!class_exists('Envato_Theme_Setup_Wizard')) {
    ...
    class Envato_Theme_Setup_Wizard {
        ...
        /** @var string Full url to server-script.php (available from https://gist.github.com/eyecix ) */
        ...
        /**
         * Complete URL to Import Demo Data
         *
         * @since 1.1.2
         *
         * @var string
        ...
        public function init_globals() {
            ...
            $this->oauth_script = apply_filters($this->theme_name . '_theme_setup_wizard_oauth_script', 'https://demo.eyecix.com/api/server-script.php');
            ...
            $this->plugin_url = jobsearch_plugin_get_url('envato_setup');
        }
        ...
        public function init_actions() {
            ...
            if (function_exists('envato_market') && 1 == 2) {
...
//            );
        ...
        public function setup_wizard_header() {
            ...
            <html xmlns="http://www.w3.org/1999/xhtml" <?php language_attributes(); ?>>
                <head>
                    ...
                    echo '<t';
                    echo 'itle>' . esc_html__('Theme &rsaquo; Import Demo Data') . '</ti' . 'tle>';
                    ?>
                ...
                <body class="envato-setup wp-core-ui">
                    <h1 id="wc-logo">
                        <a href="http://themeforest.net/user/eyecix/portfolio" target="_blank"><?php
                            $image_url = $this->get_logo_image();
                            if ($image_url) {
                                $image = '<img class="site-logo" src="%s" alt="%s" style="width:%s; height:auto" />';
                                printf(
                                        $image, $image_url, get_bloginfo('name'), $this->get_header_logo_width()
                                );
                            } else {
                                ?>
        ...
        public function envato_setup_introduction() {
            ...
            if (false && isset($_REQUEST['debug'])) {
                ...
                <ul>
                    <li>
                        <a href="<?php echo esc_url($this->get_next_step_link()); ?>"
                           class="button-primary button button-next button-large"><?php esc_html_e('Run Import Demo Data Again'); ?></a>
                    </li>
                    <li>
                        <form method="post">
        ...
        private function _content_default_get() {
            ...
            $theme_style_name = basename(get_theme_mod('dtbwp_site_style', $this->get_default_theme_style()));
            ...
            $content['users'] = array(
                ...
                'description' => esc_html__('Insert demo users.'),
                'pending' => esc_html__('Pending.'),
                'installing' => esc_html__('Installing Users.'),
                'success' => esc_html__('Success.'),
                'install_callback' => array($this, '_content_users_install'),
                'checked' => $this->is_possible_upgrade() ? 0 : 1,
                    // dont check if already have content installed.
            );
            ...
            $content['plugin_settings'] = array(
                'title' => esc_html__('Plugin Options'),
                'description' => esc_html__('Configure Plugin Options.'),
                'pending' => esc_html__('Pending.'),
                'installing' => esc_html__('Installing Plugin Options.'),
                'success' => esc_html__('Success.'),
            ...
            $content['custom_fields'] = array(
                ...
                'description' => esc_html__('Configure Custom Fields.'),
                'pending' => esc_html__('Pending.'),
                'installing' => esc_html__('Installing Custom Fields.'),
                'success' => esc_html__('Success.'),
                'install_callback' => array($this, '_custom_fields_install'),
                'checked' => $this->is_possible_upgrade() ? 0 : 1,
                    // dont check if already have content installed.
            );
        ...
        private function _content_users_install() {

            $domain = 'eyecix.com';

            $login_class = new Jobsearch_Login_Registration_Submit();

            $employers_list = get_posts(array('post_type' => 'employer', 'posts_per_page' => -1));
            $candidates_list = get_posts(array('post_type' => 'candidate', 'posts_per_page' => -1));

            //
            $jobs_list = get_posts(array('post_type' => 'job', 'posts_per_page' => -1));
            if (!empty($jobs_list)) {
                $job_count = 0;
                foreach ($jobs_list as $job_item) {
                    $job_id = $job_item->ID;
                    $employer_id = isset($employers_list[$job_count]->ID) ? $employers_list[$job_count]->ID : '';
            ...
            $candidates_data_arr = array();
            if (!empty($candidates_list)) {
                $candidate_count = 0;
                foreach ($candidates_list as $candidate_item) {

                    $post_title = $candidate_item->post_title;
                    $post_title_arr = explode(' ', $post_title);

                    $first_name = isset($post_title_arr[0]) ? $post_title_arr[0] : '';
                    $last_name = isset($post_title_arr[1]) ? $post_title_arr[1] : $first_name;

                    $email = 'cand-dummy' . $candidate_count . '@' . $domain;

                    $user_arr = array();
                    $user_arr['user_login'] = str_replace(' ', '_', $post_title);
                    $user_arr['first_name'] = $first_name;
                    $user_arr['last_name'] = $last_name;
                    $user_arr['user_pass'] = md5('demo12345');
                    $user_arr['user_email'] = $email;
                    $user_arr['nickname'] = $first_name;
                    $user_arr['display_name'] = $post_title;
                    $user_arr['role'] = 'jobsearch_candidate';
                    $user_arr['member_id'] = $candidate_item->ID;

                    $candidates_data_arr[] = $user_arr;
                    $candidate_count++;
                }
            }

            $employers_data_arr = array();
            if (!empty($employers_list)) {
                $employer_count = 0;
                foreach ($employers_list as $employer_item) {

                    $post_title = $employer_item->post_title;
                    $post_title_arr = explode(' ', $post_title);

                    $first_name = isset($post_title_arr[0]) ? $post_title_arr[0] : '';
                    $last_name = isset($post_title_arr[1]) ? $post_title_arr[1] : $first_name;

                    $email = 'emp-dummy' . $employer_count . '@' . $domain;
                    ...
                    $user_arr['nickname'] = $first_name;
                    $user_arr['display_name'] = $post_title;
                    $user_arr['role'] = 'jobsearch_employer';
                    $user_arr['member_id'] = $employer_item->ID;

                    $employers_data_arr[] = $user_arr;
                    $employer_count++;
                }
            }

            $users_array = array_merge($employers_data_arr, $candidates_data_arr);
            ...
            if (!empty($users_array)) {
                foreach ($users_array as $user) {
                    ...
                    $random_password = isset($user['user_pass']) ? $user['user_pass'] : '';
                    ...
                    if (!email_exists($email)) {
                        ...
                        if (isset($user['role']) && $user['role'] != '') {
                            if ($user['role'] == 'jobsearch_employer') {
                                wp_update_user(array('ID' => $id, 'role' => 'jobsearch_employer'));

                                if (isset($user['member_id']) && $user['member_id'] != '') {
                                    update_user_meta($id, 'jobsearch_employer_id', $user['member_id']);
                                    update_post_meta($user['member_id'], 'jobsearch_user_id', $id);
                                }
                            } else {
                                wp_update_user(array('ID' => $id, 'role' => 'jobsearch_candidate'));

                                if (isset($user['member_id']) && $user['member_id'] != '') {
                                    update_user_meta($id, 'jobsearch_candidate_id', $user['member_id']);
                                    update_post_meta($user['member_id'], 'jobsearch_user_id', $id);
                                }
                            }
                        }
        ...
        private function _fetch_remote_file($url, $post) {
            ...
            if (function_exists('wp_jobsearch_demo_data_get_path')) {
                $local_file = wp_jobsearch_demo_data_get_path('content/images/') . $file_name;
            } else {
                $local_file = jobsearch_plugin_get_path('envato_setup/content/images/stock/') . $file_name;
            }
        ...
        public function _content_install_settings() {
            ...
            $custom_options = $this->_get_json('plugin_options.json');

            if (isset($custom_options['jobsearch_plugin_options']) && class_exists('ReduxFrameworkInstances')) {
                $optionsss = json_decode($custom_options['jobsearch_plugin_options'], true);
                $redux = ReduxFrameworkInstances::get_instance('jobsearch_plugin_options');
                $redux->set_options($optionsss);
            }
            ...
            $homepage = get_page_by_title('Jobs Listing');
            if ($homepage) {
        ...
        public function _custom_fields_install() {

            $this->_handle_delayed_posts(true); // final wrap up of delayed posts.
            $this->vc_post(); // final wrap of vc posts.

            $custom_options = $this->_get_json('custom_fields.json');

            if (isset($custom_options['jobsearch_custom_field_job'])) {
                $optionsss = maybe_unserialize($custom_options['jobsearch_custom_field_job']);
                update_option('jobsearch_custom_field_job', $optionsss);
            }
            if (isset($custom_options['jobsearch_custom_field_candidate'])) {
                $optionsss = maybe_unserialize($custom_options['jobsearch_custom_field_candidate']);
                update_option('jobsearch_custom_field_candidate', $optionsss);
            }
            if (isset($custom_options['jobsearch_custom_field_employer'])) {
                $optionsss = maybe_unserialize($custom_options['jobsearch_custom_field_employer']);
                update_option('jobsearch_custom_field_employer', $optionsss);
            }

            return true;
        }

        public function _get_json($file, $path = '') {

            if ($path != '') {
                $theme_style = $path;
            } else {
                if (function_exists('wp_jobsearch_demo_data_get_path')) {
                    $theme_style = wp_jobsearch_demo_data_get_path('content/' . basename(get_theme_mod('dtbwp_site_style', $this->get_default_theme_style())) . '/');
                } else {
            ...
            if (is_file($theme_style . basename($file))) {
                ...
                if (file_exists($file_name)) {
                    ...
                    $file_content = json_encode(json_decode($file_content, true), JSON_UNESCAPED_SLASHES);

                    $replace_url_with = str_replace('/', '\\\/', home_url('/'));
                    if (strpos($file_content, 'https:\\\/\\\/eyecix.com\\\/plugins\\\/jobsearch\\\/' . basename(get_theme_mod('dtbwp_site_style', $this->get_default_theme_style())) . '\\\/') !== false) {
                        $find_url = 'https:\\\/\\\/eyecix.com\\\/plugins\\\/jobsearch\\\/' . basename(get_theme_mod('dtbwp_site_style', $this->get_default_theme_style())) . '\\\/';
                        $file_content = str_replace(array($find_url), array($replace_url_with), $file_content);
                    }
                    $replace_url_with = str_replace('/', '\/', home_url('/'));
                    if (strpos($file_content, 'https:\/\/eyecix.com\/plugins\/jobsearch\/' . basename(get_theme_mod('dtbwp_site_style', $this->get_default_theme_style())) . '\/') !== false) {
                        $find_url = 'https:\/\/eyecix.com\/plugins\/jobsearch\/' . basename(get_theme_mod('dtbwp_site_style', $this->get_default_theme_style())) . '\/';
                        ...
                        foreach ($this->site_styles as $style_name => $style_data) {
                            ...
                            <li<?php echo $style_name == $current_style ? ' class="current" ' : ''; ?>>
                                <a href="#" data-style="<?php echo esc_attr($style_name); ?>"><img
                                        src="<?php echo esc_url($this->plugin_url . '/images/' . $style_name . '/style.jpg'); ?>"></a>
                            </li>
                        <?php } ?>
                    </ul>
                </div>
                ...
                <input type="hidden" name="new_style" id="new_style" value="">
                    ...
                    <p><em>Please Note: Advanced changes to website graphics/colors may require extensive PhotoShop and Web
                            Development knowledge. We recommend hiring an expert from <a
                                href="https://support.eyecix.com/"
                                target="_blank">Eyecix</a> to assist with any advanced website changes.</em></p>
                    <div style="display: none;">
                        <img src="https://support.eyecix.com/"
                             width="1" height="1"/>
                    </div>
                        ...
                        <p>
                            <em>On the next page you will be asked to Login with your ThemeForest account and grant
                                permissions to enable Automatic Updates. If you have any questions please <a
                                    href="https://support.eyecix.com/" target="_blank">contact us</a>.</em>
                        </p>
                        <p class="envato-setup-actions step">
                            <input type="submit" class="button-primary button button-large button-next"
        ...
        public function envato_setup_help_support() {
            ...
            <p>Item Support can be accessed from <a href="https://support.eyecix.com/" target="_blank">https://support.eyecix.com/</a>
                and includes:</p>
            <ul>
        ...
        public function envato_setup_ready() {
            ...
            <a href="https://twitter.com/share" class="twitter-share-button"
               data-url="http://themeforest.net/user/eyecix/portfolio?ref=eyecix"
            ...
            <p>Please come back and <a href="http://themeforest.net/downloads" target="_blank">leave a 5-star rating</a>
                if you are happy with this theme. <br/>Follow <a href="https://twitter.com/eyecix" target="_blank">@eyecix</a>
            ...
            <div class="envato-setup-next-steps">
                <div class="envato-setup-next-steps-first">
                    ...
                    <ul>
                        <li class="setup-product"><a class="button button-primary button-large"
                                                     href="https://twitter.com/eyecix"
                                                     target="_blank"><?php esc_html_e('Follow @eyecix on Twitter'); ?></a>
                        </li>
                ...
                <div class="envato-setup-next-steps-last">
                    ...
                    <ul>
                        <li class="documentation"><a href="https://support.eyecix.com/documentation/"
                        ...
                        <li class="support"><a href="https://support.eyecix.com/"
        ...
        public function envato_market_admin_init() {
            ...
            add_settings_section(
                    envato_market()->get_option_name() . '_' . $this->envato_username . '_oauth_login', sprintf(esc_html__('Login for %s updates'), $this->envato_username), array($this, 'render_oauth_login_description_callback'), envato_market()->get_slug()
            );
            // Items setting.
            add_settings_field(
                    $this->envato_username . 'oauth_keys', esc_html__('oAuth Login'), array($this, 'render_oauth_login_fields_callback'), envato_market()->get_slug(), envato_market()->get_option_name() . '_' . $this->envato_username . '_oauth_login'
            );
        }
        ...
        public function admin_theme_auth_notice() {
            ...
            if (function_exists('envato_market')) {
                ...
                if (!$option || empty($option['oauth']) || empty($option['oauth'][$this->envato_username]) || empty($envato_items) || empty($envato_items['items']) || !envato_market()->items()->themes('purchased')) {
                    ...
                    $dissmissed_time = get_option('dtbwp_update_notice');
