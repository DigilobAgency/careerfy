...
if (!class_exists('Jobsearch_Shortcode_Candidates_Frontend')) {
    ...
    class Jobsearch_Shortcode_Candidates_Frontend {
        ...
        public function jobsearch_candidates_shortcode_callback($atts, $content = "")
        {
            ...
            $jobsearch_tax = get_query_var( 'taxonomy', '' );
            $jobsearch_tax_term = get_query_var( 'term', '' );
            ...
            if($jobsearch_tax == 'sector' && $jobsearch_tax_term){
                $atts['candidate_cat'] = $jobsearch_tax_term;
            }
            if (!isset($atts['candidate_skills'])) {
                $atts['candidate_skills'] = '';               
            }
            if($jobsearch_tax == 'skill' && $jobsearch_tax_term){
                $atts['candidate_skills'] = $jobsearch_tax_term;
            }
            if (!isset($atts['candidate_location'])) {
                $atts['candidate_location'] = '';               
            }
            if($jobsearch_tax == 'job-location' && $jobsearch_tax_term){
                $atts['candidate_location'] = $jobsearch_tax_term;
            }
             ...
             */
                ...
                if (!empty($candidate_cus_field_arr)) {
                    $candidate_cus_field_arr = maybe_serialize($candidate_cus_field_arr);
                    $candidate_cus_field_arr = base64_encode($candidate_cus_field_arr);
                }
        ...
        public function jobsearch_candidates_content($candidate_arg = '') {
            ...
            $sorting_on_profilescore = false;
            $candidate_sorting_on_score = isset($jobsearch_plugin_options['jobsearch_candidate_profilescore_sort']) ? $jobsearch_plugin_options['jobsearch_candidate_profilescore_sort'] : '';
            ...
            if ($candidate_listing_percent == 'on' && $candidate_sorting_on_score == 'on') {
                $sorting_on_profilescore = true;
            }
            ...
            $search_title = isset($_REQUEST['search_title']) ? stripslashes(sanitize_text_field($_REQUEST['search_title'])) : '';
            ...
            if ($candidate_act_orderby == 'promote_profile' || $sorting_on_profilescore === true) {
            ...
            $skill_in = !empty($atts['candidate_skills']) ? $atts['candidate_skills'] : '';
            ...
            if (isset($_REQUEST['sector_cat']) && $_REQUEST['sector_cat'] != '' && $_REQUEST['sector_cat'] != 'all') {
            ...
            $element_filter_arr = apply_filters('jobsearch_acf_listing_query_meta_fields', $element_filter_arr, 'candidate');
            $args = array(
...
//                }
            ...
            <form autocomplete="off" id="jobsearch_candidate_frm_<?php echo absint($candidate_short_counter); ?>">
                ...
                if ($listing_top_map == 'yes') {
                    ...
                    $candidate_location = isset($atts['candidate_location']) ? $atts['candidate_location'] : '';
                    $location_val = !empty($_REQUEST['location']) ? $_REQUEST['location'] : $candidate_location;

                    if(!empty($location_val)){
                        $_REQUEST['location'] = $location_val;
                    }
                    ...
                    foreach ($candidate_all_posts as $candidate_post) {
                        ...
                        if ($listing_latitude != '' && $listing_longitude != '') {
                            ...
                            $jobsearch_candidate_jobtitle = get_post_meta($candidate_post, 'jobsearch_field_candidate_jobtitle', true);
                            $candidate_company_str = '';
                            if ($jobsearch_candidate_jobtitle != '' && !$cand_profile_restrict::cand_field_is_locked('profile_fields|job_title')) {
                                $candidate_company_str .= apply_filters('jobsearch_cand_jobtitle_indisplay', $jobsearch_candidate_jobtitle, $candidate_post);
                            }
                            ...
                            //address
                            $map_posadres = jobsearch_post_city_contry_txtstr($candidate_post, $loc_view_country, $loc_view_state, $loc_view_city);
                            if ($map_posadres != '') {
                            ...
                            if ($location_map_type == 'mapbox') {
                                $list_all_atts = array(
                                    ...
                                    'properties' => array(
                                        'id' => $candidate_post,
                                        'title' => apply_filters('jobsearch_candidate_listing_item_title', wp_trim_words(get_the_title($candidate_post), 5), $candidate_post),
                                        'job_title' => $candidate_company_str,
                ...
                <?php
                if (isset($candidate_arg['custom_fields']) && !empty($candidate_arg['custom_fields'])) {
                    $candidate_arg['custom_fields'] = base64_decode($candidate_arg['custom_fields']);
                    $candidate_arg['custom_fields'] = maybe_unserialize($candidate_arg['custom_fields']);
                }
                if ($cand_top_search == 'yes') {
                    ...
                    $candidate_location = !empty($atts['candidate_location']) ? esc_html($atts['candidate_location']) : '';
                    $location_val = isset($_REQUEST['location']) ? esc_html($_REQUEST['location']) : $candidate_location;
                                            ...
                                            <li>
                                                <div class="jobsearch-select-style">
                                                    <select name="sector_cat" class="selectize-select"
                                                        ...
                                                        <option value="all"<?php echo ($cat_sector_val == 'all' ? ' selected' : '') ?>><?php esc_html_e('All', 'wp-jobsearch') ?></option>
                                                        <?php
        ...
        public function edit_join($join_paged_statement, $wp_query)
        {
            ...
            $join_to_add = "
                LEFT JOIN {$wpdb->prefix}postmeta AS score_mt
                    ON ({$wpdb->prefix}posts.ID = score_mt.post_id
                        AND score_mt.meta_key = 'promote_profile_substime')";
            if ($candidate_listing_percent == 'on' && $candidate_sorting_on_score == 'on') {
                $join_to_add = "LEFT JOIN {$wpdb->prefix}postmeta AS score_mt
                    ON ({$wpdb->prefix}posts.ID = score_mt.post_id
                        AND score_mt.meta_key = 'overall_skills_percentage')";
            }
        ...
        public function edit_orderby($orderby_statement, $wp_query)
        {
            ...
            if ($candidate_listing_percent == 'on' && $candidate_sorting_on_score == 'on') {
                $orderby_statement = "cast(score_mt.meta_value as unsigned) DESC, cast(score_mt.meta_value as unsigned) DESC, ID DESC";
            } else {
                $orderby_statement = "cast(score_mt.meta_value as unsigned) DESC, ID DESC";
            }
        ...
        public function candidate_location_filter($all_post_ids)
        {
            ...
            global $sitepress;
            $candidate_location = isset($atts['candidate_location']) ? $atts['candidate_location'] : '';
            $location_val = isset($_REQUEST['location']) ? $_REQUEST['location'] : $candidate_location;
            ...
            if (isset($_REQUEST['location']) && $_REQUEST['location'] != '') {
                ...
                $loc_decod_str = stripslashes($loc_decod_str);
                
                $tochck_meta_fields = array(
                    'jobsearch_field_location_address',
                    'jobsearch_field_location_postalcode',
                    'jobsearch_field_location_location1',
                    'jobsearch_field_location_location2',
                    'jobsearch_field_location_location3',
                    'jobsearch_field_location_location4',
                );

                $check_all_locs = false;
                if ($loc_decod_str != '' && strpos($loc_decod_str, ',')) {
                    $loc_str_exp = explode(',', $loc_decod_str);
                    if (count($loc_str_exp) > 1) {
                        $check_all_locs = true;
                        
                        //
                        $location_condition_arr = [];
                        foreach ($loc_str_exp as $loc_str_coma) {
                            $topass_metaqry_args = ['relation' => 'OR'];
                            foreach ($tochck_meta_fields as $tochck_meta_field) {
                                $topass_metaqry_args[] = array(
                                    'key' => $tochck_meta_field,
                                    'value' => trim($loc_str_coma),
                                    'compare' => 'LIKE',
                                );
                            }
                            
                            $location_condition_arr[] = $topass_metaqry_args;
                        }
                    }
                }
                ...
                if (function_exists('icl_object_id') && function_exists('wpml_init_language_switcher')) {
                    ...
                    if (empty($location_rslt) && isset($trans_able_options['candidate']) && $trans_able_options['candidate'] == '2') {
                        ...
                        $location = isset($_REQUEST['location']) ? $_REQUEST['location'] : $candidate_location;
                        if ($location != '') {
            ...
            if (isset($qrystr) && $qrystr != '') {
                ...
                if (isset($qrystr)) {
                    ...
                    foreach ($qrystr_arr as $qry_var => $qry_val) {
                        ...
                        if (strpos($qry_var, 'nonce') !== false || strpos($qry_var, 'loc_count') !== false || 'candidate_page' == $qry_var || 'candidate-listing/' == $qry_var || 'q' == $qry_var || 'lang' == $qry_var || 'tab' == $qry_var || 'cand_avail_dates' == $qry_var || 'page_id' == $qry_var || 'per-page' == $qry_var || 'action' == $qry_var || 'ajax_filter' == $qry_var || 'advanced_search' == $qry_var || 'candidate_arg' == $qry_var || 'action' == $qry_var || 'alert-frequency' == $qry_var || 'alerts-name' == $qry_var || 'loc_polygon' == $qry_var || 'alerts-email' == $qry_var || 'loc_polygon_path' == $qry_var) {
                            continue;
                        }
                        if ('candidate_salary_type' == $qry_var && !empty($job_salary_types)) {
                            if ($qry_val != '') {
                                $salary_type_val_str = '';
                                $salary_type_val = isset($qry_val[0]) ? stripslashes(sanitize_text_field($qry_val[0])) : '';
                                $slar_type_count = 1;
                                ...
                                $keyword_html .= '<li>';
                                $keyword_html .= '<a href="' . jobsearch_esc_html(jobsearch_remove_qrystr_extra_var($qrystr, $qry_var)) . '" title="' . $qry_var_str . '">' . $qry_var_str . ': ' . $salary_type_val_str . ' <i class="fa fa-close"></i></a>';
                                $keyword_html .= '</li>';
                            }
                        } else {
                            if ($qry_val != '') {
                                ...
                                if (is_array($qry_val)) {
                                    foreach ($qry_val as $qry_val_var => $qry_val_value) {
                                        if ($qry_val_value != '') {
                                            ...
                                            $qry_val_value  = stripslashes(sanitize_text_field($qry_val_value));
                                            $qry_var_str = apply_filters('jobsearch_listing_url_query_vars_result', $qry_var, 'candidate');
                                            $keyword_html .= '<li>';
                                            ...
                                            $keyword_html .= '<a href="' . wp_kses(jobsearch_remove_qrystr_extra_var($qrystr1, $qry_var), array()) . '" title="' . $qry_var_str . '">' . $qry_var_str . ': ' . jobsearch_esc_html($qry_val_str) . ' <i class="fa fa-close"></i></a>';
                                            $keyword_html .= '</li>';
                                        }
                                    }
                                } else {
                                    $qry_val  = stripslashes(sanitize_text_field($qry_val));
                                    ...
                                    $keyword_html .= '<a href="' . wp_kses(jobsearch_remove_qrystr_extra_var($qrystr, $qry_var), array()) . '" title="' . $var_title . '">' . $var_title . ': ' . jobsearch_esc_html($qry_val_str) . ' <i class="fa fa-close"></i></a>';
                                    $keyword_html .= '</li>';
                                }
                            }
                        }
