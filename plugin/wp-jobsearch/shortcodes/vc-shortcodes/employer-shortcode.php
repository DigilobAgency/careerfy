    ...
    class Jobsearch_Shortcode_Employers_Frontend
    {
        ...
        public function jobsearch_employers_shortcode_callback($atts, $content = "")
        {
            ...
            if($jobsearch_tax == 'sector' && $jobsearch_tax_term){
                $atts['employer_cat'] = $jobsearch_tax_term;
            }
            if (!isset($atts['employer_location'])) {
                $atts['employer_location'] = '';                
            }
            if($jobsearch_tax == 'job-location' && $jobsearch_tax_term){
                $atts['employer_location'] = $jobsearch_tax_term;
            }
            ...
            if (!isset($atts['employer_locfilters_sortby'])) {
                $atts['employer_locfilters_sortby'] = 'yes';
            }
            if (!isset($atts['employer_datefilters_sortby'])) {
                $atts['employer_datefilters_sortby'] = 'yes';
            }
            if (!isset($atts['employer_sectorfilters_sortby'])) {
                $atts['employer_sectorfilters_sortby'] = 'yes';
            }
            if (!isset($atts['employer_teamfilters_sortby'])) {
                $atts['employer_teamfilters_sortby'] = 'yes';
            }
        ...
        public function jobsearch_employers_content($employer_arg = '')
        {
            ...
            $element_filter_arr = apply_filters('jobsearch_acf_listing_query_meta_fields', $element_filter_arr, 'employer');
            $args = array(
                            ...
                            if (is_array($qry_val)) {
                                foreach ($qry_val as $qry_val_var => $qry_val_value) {
                                    if ($qry_val_value != '') {
                                        ...
                                        $qrystr1 = str_replace("&" . $qry_var . '=' . $qry_val_value, "", $qrystr);
                                        $keyword_html .= '<a href="' . wp_kses(jobsearch_remove_qrystr_extra_var($qrystr1, $qry_var), array()) . '" title="' . $qry_var_str . '">' . $qry_var_str . ': ' . ucwords(str_replace(array("+", "-"), " ", jobsearch_esc_html($qry_val_value))) . ' <i class="fa fa-close"></i></a>';
                                        $keyword_html .= '</li>';
                                    }
                                }
                            } else {
                                ...
                                $keyword_html .= '<a href="' . wp_kses(jobsearch_remove_qrystr_extra_var($qrystr, $qry_var), array()) . '" title="' . $var_title . '">' . $var_title . ': ' . ucwords(str_replace(array("+", "-"), " ", jobsearch_esc_html($qry_val))) . ' <i class="fa fa-close"></i></a>';
                                $keyword_html .= '</li>';
                            }
                        }
                    }
