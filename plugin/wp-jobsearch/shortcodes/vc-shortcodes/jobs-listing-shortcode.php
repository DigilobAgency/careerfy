...
if (!class_exists('Jobsearch_Shortcode_Jobs_Frontend')) {
    ...
    class Jobsearch_Shortcode_Jobs_Frontend {
        ...
        public function jobsearch_jobs_shortcode_callback($atts, $content = "") {
            global $jobsearch_gdapi_allocation, $jobseacrh_jobsh_attslist, $jobsearch_plugin_options;
            ...
            if($jobsearch_tax == 'sector' && $jobsearch_tax_term){
                $atts['job_cat'] = $jobsearch_tax_term;
            }
            if (!isset($atts['job_skills'])) {
                $atts['job_skills'] = '';                
            }
            if($jobsearch_tax == 'skill' && $jobsearch_tax_term){
                $atts['job_skills'] = $jobsearch_tax_term;
            }
            if (!isset($atts['job_location'])) {
                $atts['job_location'] = '';               
            }
            if($jobsearch_tax == 'job-location' && $jobsearch_tax_term){
                $atts['job_location'] = $jobsearch_tax_term;
            }
            ...
            if (!isset($atts['job_title_len'])) {
                $atts['job_title_len'] = '0';
            }
            ...
            if($jobsearch_tax == 'jobtype' && $jobsearch_tax_term){
                $atts['job_type'] = $jobsearch_tax_term;
            }
            ...
            if (!empty($job_cus_field_arr)) {
                $job_cus_field_arr = maybe_serialize($job_cus_field_arr);
                $job_cus_field_arr = base64_encode($job_cus_field_arr);
            }
            /*
        ...
        public function jobs_list_args($atts = array()) {
            ...
            $job_skills = isset($atts['job_skills']) ? $atts['job_skills'] : '';
            ...
            $job_location = isset($atts['job_location']) ? $atts['job_location'] : '';
            $location_val = isset($_REQUEST['location']) ? $_REQUEST['location'] : $job_location;
            ...
            if (isset($_REQUEST['job_skills']) && $_REQUEST['job_skills'] != '') {
                ...
                if(!empty($skill_in) && !is_array($skill_in)){
            ...
            if (isset($_REQUEST['job_posted_by']) && $_REQUEST['job_posted_by'] != '') {
                $base_emp_lbl = $_REQUEST['job_posted_by'];
                $base_emp_ids = jobsearch_get_post_id_by_title($base_emp_lbl, 'like');
                $base_emp_ids = !empty($base_emp_ids) && is_array($base_emp_ids) ? $base_emp_ids : array(0);
                $element_filter_arr[] = array(
                    'key' => 'jobsearch_field_job_posted_by',
                    'value' => $base_emp_ids,
                    'compare' => 'IN',
            ...
            $element_filter_arr = apply_filters('jobsearch_acf_listing_query_meta_fields', $element_filter_arr, 'job');           
        ...
        public function jobsearch_jobs_content($job_arg = '') {
            ...
            if (isset($_REQUEST['job_arg']) && $_REQUEST['job_arg']) {
                $job_arg = $_REQUEST['job_arg'];
                $job_arg = stripslashes(html_entity_decode($job_arg));
            ...
            if (!isset($atts) || (isset($atts) && empty($atts))) {
                ...
                $atts['job_locfilters_sortby'] = 'default';
                $atts['job_datefilters_sortby'] = 'default';
                $atts['job_sectorfilters_sortby'] = 'default';
                $atts['job_typefilters_sortby'] = 'default';
                $atts['job_skillfilters_sortby'] = 'default';
            ...
            global $jobsearch_global_ad_counter;
            $jobsearch_global_ad_counter = 0;
            ...
            <form autocomplete="off" id="jobsearch_job_frm_<?php echo absint($job_short_counter); ?>">
                ...
                if ($listing_top_map == 'yes') {
                    ...
                    $jobsearch_location = $_REQUEST['location'] = !empty($_REQUEST['location']) ? jobsearch_esc_html($_REQUEST['location']) : $job_location;
                    
                    if ($loc_def_adres != '' && !($jobsearch_location)) {
                    ...
                    if (!empty($jobsearch_location)) {
                        $url_get_loc = jobsearch_esc_html($jobsearch_location);
                ...
                <div style="display:none" id="job_arg<?php echo absint($job_short_counter); ?>">
                    <script type="text/javascript">
                        ...
                        $args_json_html = json_encode($jobs_arggs);
                        ?>
                        document.getElementById("job_arg<?php echo absint($job_short_counter); ?>").innerHTML = '<?php echo ($args_json_html) ?>';
                    </script>
                </div>
                ...
                <?php
                if (isset($job_arg['custom_fields']) && !empty($job_arg['custom_fields'])) {
                    $job_arg['custom_fields'] = base64_decode($job_arg['custom_fields']);
                    $job_arg['custom_fields'] = maybe_unserialize($job_arg['custom_fields']);
                }
                ...
                <div class="jobsearch-row">
                    ...
                    <div class="<?php echo esc_html($content_columns); ?>">
                        ...
                        $pagi_html = ob_get_clean();
                        echo apply_filters('jobsearch_jobs_listing_pagination_html', $pagi_html, $paging_args);
                        ?>
                    </div>
                </div>
                <?php
        ...
        public function job_search_sort_fields($atts, $job_sort_by, $job_short_counter, $view = '', $job_totnum = '', $job_per_page = '', $withtop_feature_jobnum = 0, $topfeat_postfounds = 0) {
            ...
            if ((isset($atts['job_sort_by']) && $atts['job_sort_by'] != 'no')) {
                        ...
                        if ($rss_feeds_switch != 'no') {
                            ...
                            if (!empty($atts)) {
                                ...
                                foreach ($atts as $_q_atts_key => $_q_atts_val) {
                                    if ($_q_atts_val == '' || is_array($_q_atts_val)) {
                                        continue;
                                    }
                ...
                //
                echo '</div>';
                $sort_con_html = ob_get_clean();
                
                echo apply_filters('jobsearch_jobs_listin_top_sort_bar_html', $sort_con_html, $atts, $job_sort_by, $job_short_counter, $view, $job_totnum, $job_per_page, $withtop_feature_jobnum, $topfeat_postfounds);
        ...
        public function location_radius_filter_ids() {
            ...
            if (isset($_REQUEST['loc_radius']) && $_REQUEST['loc_radius'] > 0 && $exist_loc) {
                ...
                $countries_file = jobsearch_plugin_get_path('modules/locations/data/countries.json');
...
//                    }
        ...
        public function jobsearch_job_pagination_callback($args) {
            ...
            if ($show_pagination <> 'yes') {
                ...
                $html .= '<div class="' . apply_filters('jobsearch_job_listin_pagination_main_class', 'jobsearch-pagination-blog') . '"><ul class="jobsearch-page-numbers">';
                if ($paged_id > 1) {
                    $html .= '<li class="prev-pli-con">'
                    ...
                    $html .= '<span><i class="' . apply_filters('jobsearch_job_listin_pagination_prev_icon', 'jobsearch-icon jobsearch-arrows4') . '"></i></span>' . apply_filters('jobsearch_job_listin_pagination_prev_txt', '') . ''
                            . '</a>'
                            . '</li>';
                } else {
                ...
                if ($total_posts > 0 && $job_per_page > 0 && $paged_id < ($total_posts / $job_per_page)) {
                    ...
                    $html .= apply_filters('jobsearch_job_listin_pagination_after_txt', '') . '<span><i class="' . apply_filters('jobsearch_job_listin_pagination_next_icon', 'jobsearch-icon jobsearch-arrows4') . '"></i></span>'
                            . '</a>'
                            . '</li>';
                } else {
