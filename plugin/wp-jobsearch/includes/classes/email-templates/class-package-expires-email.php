        ...
        if (!empty($ordrs_posts)) {
            foreach ($ordrs_posts as $order) {
                ...
                if (
                    ...
                    $order_user_id = $order->get_meta('jobsearch_order_user');
                    
                    $order_user_obj = get_user_by('ID', $order_user_id);
                    
                    if (($package_type == 'job' && jobsearch_pckg_order_is_expired($order_id)) || ($package_type == 'emp_allin_one' && jobsearch_allinpckg_order_is_expired($order_id))) {
                        do_action('jobsearch_jobs_package_expire_email', $order_user_obj, $order_id);
                        $order->update_meta_data('jobsearch_ordrexpiry_mail_sent', 'sent');
                        $order->save();
                        update_post_meta($order_id, 'jobsearch_ordrexpiry_mail_sent', 'sent');
                    }
                    if (($package_type == 'featured_jobs' || $package_type == 'featured_tags') && jobsearch_fjobs_pckg_order_is_expired($order_id)) {
