                    ...
                    <div class="jobsearch-employer-box-section">
                        <div class="jobsearch-profile-title">
                            ...
                            <a href="<?php echo add_query_arg(array('tab' => 'all-applicants'), $page_url) ?>"
                               class="applicnts-view-btn"><i class="jobsearch-icon jobsearch-company-workers"></i> <?php esc_html_e('Internal Applicants', 'wp-jobsearch') ?></a>
                            <?php
                                    ...
                                    <ul>
                                        ...
                                        if (is_array($file_path) && count($file_path) > 1) {
                                            if (isset($file_path[1])) {
                                                $att_zero_file_path = $file_path[1];
                                                $filename = basename($att_zero_file_path);
                                                $downlod_label = esc_html__('Download CV', 'wp-jobsearch');
                                                $file_url = apply_filters('wp_jobsearch_email_cvfile_downlod_url', $att_file_path, 1, $app_id);
                                                ?>
                                                <li><a href="<?php echo ($file_url) ?>" class="preview-candidate-profile btn-downlod-cvbtn" oncontextmenu="javascript: return false;" onclick="javascript: if ((event.button == 0 && event.ctrlKey)) {
                                                                                ...
                                                                                ;" download="<?php echo ($filename) ?>"><?php echo ($downlod_label) ?></a></li>
                                                <?php
                                            }
                                            if (isset($file_path[0])) {
                                                $att_zero_file_path = $file_path[0];
                                                $filename = basename($att_zero_file_path);
                                                $downlod_label = esc_html__('Download Cover', 'wp-jobsearch');
                                                $file_url = apply_filters('wp_jobsearch_email_cvfile_downlod_url', $att_file_path, 0, $app_id);
                                                ?>
                                                <li><a href="<?php echo ($file_url) ?>" class="preview-candidate-profile btn-downlod-cvbtn" oncontextmenu="javascript: return false;" onclick="javascript: if ((event.button == 0 && event.ctrlKey)) {
                                                                                    return false
                                                                                }
                                            ...
                                            if (isset($att_file_args['file_url']) && $att_file_args['file_url'] != '' && $app_id > 0 && $file_path != '' && file_exists($file_path)) {
                                                ...
                                                $downlod_label = esc_html__('Download CV', 'wp-jobsearch');
                                                if (strpos($filename, 'cover_') !== false) {
                                                    $downlod_label = esc_html__('Download Cover', 'wp-jobsearch');
                                                }
                                                ...
                                                <li><a href="<?php echo($file_url) ?>"
                                                    ...
                                                    download="<?php echo($filename) ?>"><?php echo ($downlod_label) ?></a>
                                                </li>
                                                <?php
                                            }
                                        }
